...
const Navbar = ({
  ...
  //DOESNT WORK RN, EXCEEDED THE LIMIT OF API CALLS

  /* const synchroData = useMemo(async () => {
      searchValue.length >= 3 && handleData(await getData(searchValue));
      console.log(data);

      return handleData.cancel();
    }, [handleData, searchValue]);

    const debouncedSearch = debounce(() => synchroData(), 400); */
  ...
  useEffect(() => {
    ...
    const debouncedSr = debounce(() => debouncedSearch(), 400);
