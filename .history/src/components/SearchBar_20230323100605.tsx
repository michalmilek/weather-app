...
import { getData, LocationResponseInterface } from "../utils/fetchingData";
...
const SearchBar = ({ handleSearchValue, searchValue, handleData }: Props) => {
    ...
    <Box sx={{ minWidth: "400px", position: "relative" }}>
      ...
      {Object.keys(data).length !== 0 && searchOn && (
        <Box
          ...
          {data?.map((item: any) => (
            <div key={item.lat + item.lon + item.country}>
              <Typography
                onClick={() =>
                  handleLocation({
                    lat: item.lat,
