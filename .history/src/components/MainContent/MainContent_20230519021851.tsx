...
import useWeatherForUpcomingDays from "../../hooks/useWeatherForUpcomingDays";
...
const MainContent = ({ location, currentTemp }: Props) => {
  ...
  const addCity = (city: string, lonN: number, latN: number) => {
    const addedArray = [...cities, { name: city, lon: lonN, lat: latN }];
    const nonDuplicatedCities = uniqBy(addedArray, "name");
  ...
  const {
    data: fetchedDataForUpcomingDays,
    isFetching,
    isLoading,
    isError,
    error,
  } = useWeatherForUpcomingDays(location, currentTemp, isModalOpen);
